...
import styles from "./MonthCalendarStyle.module.css";
    ...
    <div className={styles.monthCalendar}>
      <div className={styles.monthCalendarHeader}>
        {shortWeekDays.map((day, index) => (
          <div key={index} className={styles.monthCalendarHeading}>
            <h3>{day}</h3>
          </div>
        ))}
      </div>
      ...
      <div className={styles.monthCalendarBody}>
        <table className={styles.monthCalendarTable}>
            ...
            <tr key={rowIndex}>
              {selectedMonths
                ...
                .map((date, index) => (
                  <td
                    key={index}
                    className={date.getDate() === 1 ? styles.newMonth : ""}
                  >
                    <h2>
                      {date.getDate()}{" "}
                      {date.getDate() === 1
